/*
  ...
  1. Changes
    ...
    - Fix constraint handling for email uniqueness
    
  2. Security
    - Maintain existing constraints
    - Ensure proper index coverage
*/
...
CREATE INDEX IF NOT EXISTS submissions_pending_idx 
  ON public.event_submissions (created_at)
...
CREATE INDEX IF NOT EXISTS company_pending_idx 
  ON public.company_submissions (created_at)
  WHERE status = 'pending';

CREATE INDEX IF NOT EXISTS consortium_pending_idx 
  ON public.consortium_submissions (created_at)
  WHERE status = 'pending';

CREATE INDEX IF NOT EXISTS innovation_pending_idx 
  ON public.innovation_submissions (created_at)
  WHERE status = 'pending';