...
interface ConsortiumSubmission {
  id: string;
  name: string;
  website?: string;
  focus_area?: string;
...
export default function ConsortiumSubmissions() {
  const [submissions, setSubmissions] = useState<ConsortiumSubmission[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState('all');
  const [editingId, setEditingId] = useState<string | null>(null);
  const [editData, setEditData] = useState<Partial<ConsortiumSubmission>>({});
  ...
  const fetchSubmissions = async () => {
    try {
     ...
     const { data, error } = await client
      ...
      if (error) {
        console.error('Error fetching consortium submissions:', error);
        if (error.message.includes('Not authenticated')) {
          setError('Authentication required to view submissions');
        } else {
          setError('Failed to load consortium submissions');
        }
        setSubmissions([]);
        return;
      }
  ...
  const startEdit = (submission: ConsortiumSubmission) => {
  ...
  const filteredSubmissions = submissions.filter(submission => {
    const matchesSearch = searchTerm === '' || 
      ...
      (submission.focus_area && submission.focus_area.toLowerCase().includes(searchTerm.toLowerCase()));
    ...
    <div className="space-y-6">
      ...
      {filteredSubmissions.length > 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Consortium</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Focus & Partner</th>
                    ...
                    <td className="px-6 py-4">
                      {editingId === submission.id ? (
                        <div className="space-y-2">
                          <input
                            type="text"
                            value={editData.focus_area || ''}
                            onChange={(e) => setEditData({ ...editData, focus_area: e.target.value })}
                            className="block w-full text-sm border border-gray-300 rounded-md px-3 py-2"
                            placeholder="Focus area"
                          />
                          <input
                            type="text"
                            value={editData.government_partner || ''}
                            onChange={(e) => setEditData({ ...editData, government_partner: e.target.value })}
                            className="block w-full text-sm border border-gray-300 rounded-md px-3 py-2"
                            placeholder="Government partner"
                          />
                        </div>
                      ) : (
                        <div>
                          {submission.focus_area && (
                            <div className="text-sm text-gray-900">{submission.focus_area}</div>
                          )}
                          {submission.government_partner && (
                            <div className="text-sm text-gray-500">{submission.government_partner}</div>
                          )}
        ...
        <div className="text-center py-8 text-gray-500">
          {searchTerm || statusFilter !== 'all' ? 'No consortiums match your search criteria' : 'No consortium submissions found'}
        </div>
      )}
    </div>
  );
