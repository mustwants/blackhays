...
const ConsortiumSubmissions: React.FC<ConsortiumSubmissionsProps> = ({ initialData = [] }) => {
  ...
  const fetchSubmissions = async () => {
    try {
      ...
      setSubmissions(data || []);
      if (!data?.length) {
        setError('No consortium submissions found');
      }
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-lg font-bold">Consortium Submissions</h3>
        <button 
      ...
      {loading && !editingItem ? (
          ...
          <div key={submission.id} className="bg-white border rounded-lg p-4 shadow-sm">
            <div className="flex justify-between items-start">
              <div>
                ...
                <p className="text-sm text-bhgray-600">Focus: {submission.focus_area}</p>
                <p className="text-sm text-bhgray-600">Gov Partner: {submission.government_partner}</p>
              ...
              <div className="mt-4 space-y-3 border-t pt-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      type="text"
                      value={editingItem.focus_area || ''}
                      onChange={(e) => setEditingItem({...editingItem, focus_area: e.target.value})}
                      className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-bhred focus:ring-bhred sm:text-sm"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      type="text"
                      value={editingItem.government_partner || ''}
                      onChange={(e) => setEditingItem({...editingItem, government_partner: e.target.value})}
                      className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-bhred focus:ring-bhred sm:text-sm"
                    />
                  </div>
                </div>
        ...
        <div className="text-center py-8 text-gray-500">
          No consortium submissions found
        </div>
      )}
    </div>
  );
...
export default ConsortiumSubmissions;
