...
interface InnovationSubmissionsProps {
  initialData?: any[];
}

const InnovationSubmissions: React.FC<InnovationSubmissionsProps> = ({ initialData = [] }) => {
  ...
  const fetchSubmissions = async () => {
    try {
      ...
      setSubmissions(data || []);
      if (!data?.length) {
        setError('No innovation submissions found');
      }
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-lg font-bold">Innovation Submissions</h3>
        <button 
      ...
      {loading && !editingItem ? (
          ...
          <div key={submission.id} className="bg-white border rounded-lg p-4 shadow-sm">
            <div className="flex justify-between items-start">
              <div>
                ...
                <p className="text-sm text-bhgray-600">Type: {submission.type}</p>
                <p className="text-sm text-bhgray-600">Focus: {submission.focus_areas}</p>
                <p className="text-sm text-bhgray-600">Location: {submission.headquarters}</p>
            ...
            {editingItem && editingItem.id === submission.id && (
              <div className="mt-4 space-y-3 border-t pt-3">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700">Primary Sponsor</label>
                  <input
                    type="text"
                    value={editingItem.primary_sponsor || ''}
                    onChange={(e) => setEditingItem({...editingItem, primary_sponsor: e.target.value})}
                    className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-bhred focus:ring-bhred sm:text-sm"
                  />
                </div>
                <div>
        ...
        <div className="text-center py-8 text-gray-500">
          No innovation submissions found
        </div>
      )}
    </div>
  );
